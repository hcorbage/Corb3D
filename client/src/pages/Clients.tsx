          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {clientCNPJ.loading && <p className="text-[10px] text-blue-500 mt-0.5">Consultando CNPJ...</p>}
                ...
                {clientCNPJ.filled.length > 0 && (
                  <p className="text-[10px] text-green-600 flex items-center gap-1 mt-0.5">
                    <svg className="w-3 h-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>
                    Dados preenchidos automaticamente
                    {clientCNPJ.data?.status && <span className={`px-1 rounded text-[9px] font-semibold ${clientCNPJ.data.active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>{clientCNPJ.data.status}</span>}
                  </p>
                )}
              </div>
              <div>
