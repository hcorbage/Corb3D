...
## Recent Changes
- 2026-04-01: Regra caixa fechado (opção 1): isTodayCashClosed() bloqueia criação de cash_entries via POST /api/cash-entries (409), POST /api/order-payments e POST /api/stock-movements quando caixa do dia está fechado; Inventory.tsx trata 409 com stockMovement (estoque salvo, caixa bloqueado); CashBook.tsx mostra banner amber + botão desabilitado quando caixa fechado
- 2026-04-01: Consistência financeira: (1) DELETE /api/order-payments agora cascade-deleta o cash_entry correspondente e recalcula amountPaid/status no order_financial; (2) scheduler auto-fecha caixas órfãos abertos de datas passadas; (3) normalizeCashType() em routes valida/sanitiza type antes de salvar cash_entries
- 2026-03-31: Estoque evoluído: stockMovements table (entrada/saída/ajuste/pedido com previousQuantity+newQuantity); minQuantity field on stockItems (threshold for low stock alert); GET /api/stock-movements/:stockItemId + POST /api/stock-movements endpoints; optional Livro Caixa entry on purchase; Inventory.tsx with MovementModal (Entrada/Saída/Ajuste), HistoryModal, NewMaterialTypeModal, StockBadge (Zerado/Baixo), summary banners; Calculator.tsx stock availability warning (Zerado / Insuf.) per item row
- 2026-03-31: Menu FINANCEIRO reorganizado: nova ordem Caixa→Visão Geral→Financeiro por Pedido→Comissões→Rel. Clientes→Relatórios→Livro Caixa; "Caixa Diário" renomeado para "Caixa"; Comissões movida para dentro do grupo FINANCEIRO; não-admins veem grupo FINANCEIRO (só Comissões); bottom nav mobile dinâmico por role
